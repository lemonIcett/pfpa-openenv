...
class PFPAGrader:
    ...
    Cumulative reward is capped at 0.9 to ensure scores stay strictly in (0, 1).
    """

    # Maximum cumulative reward - kept strictly below 1.0 for validator compliance
    # Using 0.9 to provide safety margin and ensure never reaches 1.0
    MAX_CUMULATIVE_REWARD = 0.9
    ...
    def grade(
        ...
        # Clip per-step reward to [0.0, 1.0]
        reward = round(max(0.0, min(1.0, reward)), 4)
        
        # Cap cumulative reward at MAX_CUMULATIVE_REWARD (0.9) to ensure strictly < 1.0
        new_cumulative = state.cumulative_reward + reward
        state.cumulative_reward = round(
            min(self.MAX_CUMULATIVE_REWARD, new_cumulative), 4
        )
        state.step_number += 1
    ...
    def _check_done(self, state: EnvironmentState) -> bool:
        ...
        if not pending_preds and not urgent_signals:
            return True
        # Max reward reached (using the capped value)
        if state.cumulative_reward >= self.MAX_CUMULATIVE_REWARD:
            return True
        return False